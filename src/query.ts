...
export const toQuerySpec = (form: FilterForm) =>
  Effect.gen(function* () {
    ...
    if (until <= from) return yield* new ValidationError({ message: "Until must be later than From" })
    return { ...form, from, until } satisfies QuerySpec
  })

const textFilters = ["domain", "clientIp", "clientName", "upstream", "type", "status", "reply", "dnssec"] as const
...
export const serializeQuery = (spec: QuerySpec, start = 0, cursor?: number, length = 10_000) => {
  ...
  for (const key of textFilters) if (spec[key].trim() !== "") params.set(apiNames[key], spec[key].trim())
...
export const defaultFilename = (spec: QuerySpec) => {
  ...
  const suffix = textFilters.flatMap((key) => spec[key].trim() === "" ? [] : [`${key}-${spec[key].replace(/[^a-z0-9*.-]+/gi, "_")}`])
