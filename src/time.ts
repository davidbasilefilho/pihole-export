...
export const localToEpochSeconds = (input: string, timezone: string) =>
  Effect.gen(function* () {
    ...
    if (match === null) {
      return yield* new ValidationError({ message: `Invalid date/time “${input}”; use YYYY-MM-DD HH:mm[:ss]` })
    }
    const [, y, month, day, hour, minute, second = "0"] = match
    if (y === undefined || month === undefined || day === undefined || hour === undefined || minute === undefined) {
      return yield* new ValidationError({ message: `Invalid date/time “${input}”` })
    }
    const zoned = DateTime.makeZoned(
      { year: Number(y), month: Number(month), day: Number(day), hours: Number(hour), minutes: Number(minute), seconds: Number(second) },
      { timeZone: timezone, adjustForTimeZone: true, disambiguation: "reject" },
    )
    return yield* Option.match(zoned, {
      onNone: () => new ValidationError({ message: `Invalid local time or timezone: ${input} [${timezone}]` }),
      onSome: (value) => Effect.succeed(DateTime.toEpochMillis(value) / 1000),
    })
  })

export const defaultRange = (timezone = Intl.DateTimeFormat().resolvedOptions().timeZone || "UTC") => {
